...
local function set_page_count_caption(player)
    if not player.gui.left.fjei_main_window then
        return
    end
    if not global.fjei.player_data[player.index].filtered_list then
        Functions.set_filtered_list(player)
...
local function get_active_recipe_name(player)
    ...
    if not fjei_recipe_window then
        return
    end
    if not fjei_recipe_window.recipe_container then
        return
    end
...
local function add_sprite_icon(element, name, is_recipe, use_localised_name)
    ...
    if use_localised_name then
        sprite =
            element.add(
            {type = 'sprite', name = name, sprite = sprite_type .. '/' .. name, tooltip = get_localised_name(name)}
        )
    else
...
local function refresh_recipe_bar(player, selected_recipe)
    ...
    if not old_recipe_name then
        return
    end

    if not player.gui[recipe_window_position]['fjei_recipe_window'] then
        return
    end
    if not player.gui[recipe_window_position]['fjei_recipe_window'].header_container then
        return
    end
    local fjei_recipe_window_select_table =
        player.gui[recipe_window_position]['fjei_recipe_window'].header_container.header_table.scroll_pane.fjei_recipe_window_select_table
...
local function draw_recipe_window_header(player, container, item_name, recipes, mode, recipe_name)
    ...
    if mode == 1 then
        element.caption = 'Product\nof recipe: '
    else
        element.caption = 'Ingredient\nin recipe: '
    end
...
local function create_recipe_window(item_name, player, button, selected_recipe)
    ...
    if not global.fjei.item_list[item_name] then
        return
    end
    ...
    if selected_recipe then
        for k, v in pairs(recipes) do
            if v == selected_recipe then
                recipe_name = recipes[k]
            end
        end
    end
    ...
    local frame =
        player.gui[recipe_window_position].add({type = 'frame', name = 'fjei_recipe_window', direction = 'vertical'})
...
local function add_to_history(item_name, player)
    ...
    if player_data.size_of_history > column_count then
        player_data.history[player_data.size_of_history - column_count] = nil
    end
end
...
local function show_cursor_stack_item(element, player, button)
    ...
    if not global.fjei.item_list[cursor_stack.name] then
        return
    end
...
local function toggle_main_window(element, player, button)
    if show_cursor_stack_item(element, player, button) then
        return true
    end
    if player.gui.left.fjei_main_window then
        player.gui.left.fjei_main_window.destroy()
...
function Public.gui_click_actions(element, player, button)
    if not gui_actions[element.name] then
        return
    end
    if not global.fjei then
        Functions.build_tables()
...
function Public.open_recipe(element, player, button)
    ...
    if element.parent.name == 'fjei_recipe_window_selected_recipe' then
        return
    end
    if element.parent then
        if element.parent.parent then
            if element.parent.parent.name == 'fjei_recipe_window_select_table' then
                selected_recipe = item_name
            end
        end
    end
    if
        element.parent.name == 'fjei_main_window_item_list_table' or
            element.parent.name == 'fjei_main_window_history_table'
     then
        local recipe_window = player.gui[recipe_window_position]['fjei_recipe_window']
        if recipe_window then
    ...
    if not create_recipe_window(item_name, player, button, selected_recipe) then
        return
    end
    display_history(player)
