...
local function satellite_score_toggle_button(player)
    ...
    local button =
        player.gui.top.add {
        name = 'satellite_score_toggle_button',
        type = 'sprite-button',
        sprite = 'item/satellite',
        tooltip = 'Satellites in Space'
    }
...
local function satellites_in_space_gui(player)
    ...
    local progress =
        (global.satellites_in_space - global.satellite_score[i].goal) /
        (global.satellite_score[i + 1].goal - global.satellite_score[i].goal)
    if progress > 1 then
        progress = 1
    end
    ...
    local label =
        frame.add(
        {type = 'label', caption = global.satellites_in_space .. '/' .. tostring(global.satellite_score[i + 1].goal)}
    )
...
local function on_rocket_launched(event)
    ...
    if
        (global.satellites_in_space < 10) or
            ((global.satellites_in_space < 50) and ((global.satellites_in_space % 5) == 0)) or
            ((global.satellites_in_space % 25) == 0)
     then
...
local function init()
    ...
    global.satellite_score = {
        ...
        {
            goal = 40000,
            rank = 'Blue Supergiant',
            color = {r = 130, g = 130, b = 255},
            msg = '',
            msg2 = '',
            achieved = false
        },
