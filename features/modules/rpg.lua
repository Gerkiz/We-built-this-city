...
local function update_char_button(player)
	if not mod(player).rpg_button then
...
local function draw_gui(player)
	...
	local frame = left.add {type = 'frame', name = 'rpg', direction = 'vertical', caption = 'RPG'}
	...
	local value =
		(player.force.manual_crafting_speed_modifier + player.character_crafting_speed_modifier + 1) * 100 .. '%'
	add_gui_stat(tt, value, w2)

	add_gui_description(tt, ' ', w0)
	add_gui_description(tt, 'RUNNING\nSPEED', w1)
	local value =
		(player.force.character_running_speed_modifier + player.character_running_speed_modifier + 1) * 100 .. '%'
	add_gui_stat(tt, value, w2)
...
local function on_gui_click(event)
	...
	if not rpg_t[player.index][name] then
		return
	end
	if not player.character then
		return
