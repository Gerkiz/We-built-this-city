...
local function roll_target()
    local r_max = 0
    local towny = TownyTable.get('towny')
    local town_centers = towny.town_centers_placeholders

    --Skip Towns that are too low in research for the current biter evolution.
    local research_threshold = 5
...
function Public.validate_swarms()
    local towny = TownyTable.get('towny')
    for k, swarm in pairs(towny.swarms) do
        if not is_swarm_valid(swarm) then
            table_remove(towny.swarms, k)
        end
    end
end
...
function Public.wipe_units_out_of_evo_range()
    local towny = TownyTable.get('towny')
    if towny.disable_wipe_units_out_of_evo_range then
        return
    end
    local units_to_wipe = {}
...
function Public.swarm()
    local count = 0
    local towny = TownyTable.get('towny')
    for k, _ in pairs(towny.swarms) do
        count = count + 1
    end
    if count > 6 then
        return
    ...
    if not spawner then
        spawner = get_random_close_spawner(surface, market)
        if not spawner then
            return
        end
    end
    ...
    if not units[1] then
        units = spawner.surface.find_enemy_units(spawner.position, 160, market.force)
        if not units[1] then
            return
        end
    end
    ...
    if not unit_group_position then
        unit_group_position = surface.find_non_colliding_position('market', units[1].position, 256, 1)
        if not unit_group_position then
            return
        end
    end
    local unit_group = surface.create_unit_group({position = unit_group_position, force = units[1].force})
    local count = (town_center.research_counter * 1.5) + 12
    ...
    table_insert(towny.swarms, {group = unit_group, timeout = game.tick + 36000})
end

function Public.swarm_non_markets()
    local count = 0
    local towny = TownyTable.get('towny')
    for k, _ in pairs(towny.swarms) do
        count = count + 1
    ...
    local town_center = roll_target()
    if not town_center then
        return
    end
    local market = town_center.market
    local surface = town_center.surface
    local spawner = get_random_close_spawner(surface, market)
    if not spawner then
        spawner = get_random_close_spawner(surface, market)
        if not spawner then
