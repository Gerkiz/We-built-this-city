...
local function can_force_accept_member(force)
    ...
    local size_of_town_centers = towny.size_of_town_centers
...
function Public.set_town_color(event)
    ...
    local towny = TownyTable.get('towny')
    local town_center = towny.town_centers[player.name]
    if not town_center then
...
local function ally_outlander(player, target)
    ...
    if requesting_force.index == 1 then
        ...
        if target_player then
            if towny.requests[target_player.index] then
                if towny.requests[target_player.index] == player.name then
                    if towny.town_centers[target_force.name] then
                        if not can_force_accept_member(target_force) then
                            return true
                        end
                        game.print('>> ' .. player.name .. ' has settled in ' .. target_force.name .. "'s Town!", {255, 255, 0})
                        Public.add_player_to_town(player, towny.town_centers[target_force.name])
                        return true
                    end
                end
            end
    ...
    if target_force.index == 1 then
        ...
        towny.requests[player.index] = target_player.name

        if towny.requests[target_player.index] then
            if towny.requests[target_player.index] == player.force.name then
                ...
                Public.add_player_to_town(target_player, towny.town_centers[player.force.name])
                return true
            end
        end
...
local function ally_neighbour_towns(player, target)
    ...
    TownyTable.add_to_alliances(requesting_force, target_force)
...
function Public.declare_war(player, item)
    ...
    if requesting_force.name == target_force.name then
        if player.name ~= target.force.name then
            ...
            towny.requests[player.index] = nil
        end
        if player.name == target.force.name then
            if target.type ~= 'character' then
                return
    ...
    TownyTable.remove_from_alliances(requesting_force, target_force.name)
    TownyTable.remove_from_alliances(target_force, requesting_force.name)
