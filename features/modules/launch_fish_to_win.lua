...
local function fish_in_space_gui(player)
    ...
    local fish_label =
        frame.add(
        {type = 'label', caption = global.fish_in_space .. '/' .. tostring(global.catplanet_goals[i + 1].goal)}
    )
    fish_label.style.font_color = {r = 0.33, g = 0.66, b = 0.9}

    if global.catplanet_goals[i].rank then
        local rank_label = frame.add({type = 'label', caption = '  ~Rank~'})
        rank_label.style.font_color = {r = 0.75, g = 0.75, b = 0.75}
        local goals_label = frame.add({type = 'label', caption = global.catplanet_goals[i].rank})
        goals_label.style.font = 'default-bold'
        goals_label.style.font_color = global.catplanet_goals[i].color
    end
end
