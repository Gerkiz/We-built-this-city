...
local Constants = require 'features.modules.immersive_cargo_wagons.constants'
...
function Public.create_wagon_room(icw, wagon)
    ...
    if wagon.entity.type == 'locomotive' then
        ...
        local position = {
            x = area.left_top.x + (area.right_bottom.x - area.left_top.x) * 0.5,
            y = area.left_top.y + (area.right_bottom.y - area.left_top.y) * 0.5
        }
    ...
    if wagon.entity.type == 'cargo-wagon' then
        ...
        local position = {
            math_random(area.left_top.x + 2, area.right_bottom.x - 3),
            math_random(area.left_top.y + 5, area.right_bottom.y - 6)
        }
