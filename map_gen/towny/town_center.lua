local Team = require 'map_gen.towny.team'
local Public = {}
...
local function draw_town_spawn(player_name)
    ...
    local area = {
        {position.x - (town_radius + 1), position.y - (town_radius + 1)},
        {position.x + (town_radius + 1), position.y + (town_radius + 1)}
    }
    ...
    local area = {
        {position.x - town_radius * 1.5, position.y - town_radius * 1.5},
        {position.x + town_radius * 1.5, position.y + town_radius * 1.5}
    }
...
local function is_valid_location(surface, entity)
    ...
    local area = {
        {entity.position.x - town_radius, entity.position.y - town_radius},
        {entity.position.x + town_radius, entity.position.y + town_radius}
    }
    local count = 0
...
function Public.found(event)
    ...
    if inventory.get_item_count('small-plane') < 1 then
        return
    end
            ...
            surface.create_entity(
                {
                    name = 'flying-text',
                    position = entity.position,
                    text = 'Town founding is on cooldown for ' ..
                        math.ceil((global.towny.cooldowns[player.index] - game.tick) / 3600) .. ' minutes.',
                    color = {r = 0.77, g = 0.0, b = 0.0}
                }
            )
    ...
    town_center.chunk_position = {
        math.floor(town_center.market.position.x / 32),
        math.floor(town_center.market.position.y / 32)
    }
