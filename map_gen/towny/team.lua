...
local function can_force_accept_member(force)
    ...
    if #force.connected_players >= member_limit then
        game.print(
            '>> Town ' .. force.name .. ' has too many settlers! Current limit (' .. member_limit .. ')',
            {255, 255, 0}
        )
        return
    end
...
local function ally_outlander(player, target)
            ...
            if global.towny.requests[target_player.index] then
                if global.towny.requests[target_player.index] == player.name then
                    if global.towny.town_centers[target_force.name] then
                        ...
                        game.print(
                            '>> ' .. player.name .. ' has settled in ' .. target_force.name .. "'s Town!",
                            {255, 255, 0}
                        )
    ...
    if target_force.index == 1 then
        ...
        if global.towny.requests[target_player.index] then
            if global.towny.requests[target_player.index] == player.force.name then
                ...
                if player.force.name == player.name then
                    game.print(
                        '>> ' .. player.name .. ' has accepted ' .. target_player.name .. ' into their Town!',
                        {255, 255, 0}
                    )
                else
                    game.print(
                        '>> ' ..
                            player.name ..
                                ' has accepted ' .. target_player.name .. ' into' .. player.force.name .. "'s Town!",
                        {255, 255, 0}
                    )
                end
        ...
        if player.force.name == player.name then
            game.print(
                '>> ' .. player.name .. ' is inviting ' .. target_player.name .. ' into their Town!',
                {255, 255, 0}
            )
        else
            game.print(
                '>> ' ..
                    player.name .. ' is inviting ' .. target_player.name .. ' into ' .. player.force.name .. "'s Town!",
                {255, 255, 0}
            )
        end
        return true
...
local function ally_neighbour_towns(player, target)
    ...
    game.print(
        '>> Town ' .. requesting_force.name .. ' has set ' .. target_force.name .. ' as their friend!',
        {255, 255, 0}
    )

    if target_force.get_friend(requesting_force) then
        game.print(
            '>> The towns ' .. requesting_force.name .. ' and ' .. target_force.name .. ' have formed an alliance!',
            {255, 255, 0}
        )
    end
end
...
function Public.ally_town(player, item)
    ...
    local area = {
        {position.x - item_drop_radius, position.y - item_drop_radius},
        {position.x + item_drop_radius, position.y + item_drop_radius}
    }
    local requesting_force = player.force
    local target = false
...
function Public.declare_war(player, item)
    ...
    if requesting_force.name == target_force.name then
        ...
        if player.name == target.force.name then
            ...
            game.print(
                '>> ' .. player.name .. ' has banished ' .. target_player.name .. ' from their Town!',
                {255, 255, 0}
            )
            global.towny.requests[player.index] = nil
        end
    ...
    game.print(
        '>> ' ..
            player.name ..
                ' has dropped the coal! Town ' ..
                    target_force.name .. ' and ' .. requesting_force.name .. ' are now at war!',
        {255, 255, 0}
    )
end
...
local function delete_chart_tag_for_all_forces(market)
    ...
    for _, force in pairs(forces) do
        local tags =
            force.find_chart_tags(surface, {{position.x - 0.1, position.y - 0.1}, {position.x + 0.1, position.y + 0.1}})
        local tag = tags[1]
        if tag then
...
function Public.add_chart_tag(force, market)
    ...
    local tags =
        ...
        market.surface,
        {{position.x - 0.1, position.y - 0.1}, {position.x + 0.1, position.y + 0.1}}
    )
    if tags[1] then
        return
    end
    force.add_chart_tag(
        market.surface,
        {icon = {type = 'item', name = 'stone-furnace'}, position = position, text = market.force.name .. "'s Town"}
    )
end
...
function Public.kill_force(force_name)
    ...
    game.print(
        '>> ' ..
            force_name ..
                "'s town has fallen! [gps=" ..
                    math.floor(market.position.x) .. ',' .. math.floor(market.position.y) .. ']',
        {255, 255, 0}
    )
end
