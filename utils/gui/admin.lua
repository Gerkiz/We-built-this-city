...
local function jail(player, source_player)
    ...
    if source_role.power < target_role.power then
        source_player.print(player.name .. " has a higher role than you! Can't jail!", Color.warning)
        return
    end
    if source_role.power == target_role.power then
        source_player.print(player.name .. " has an equal role! Can't jail!", Color.warning)
        return
    end
    ...
    if package.loaded['map_gen.multiplayer_spawn.main'] then
        buttons[#buttons + 1] =
            t.add(
            {
                ...
                name = 'respawn_player',
                tooltip = 'Will reset and respawn the player'
            }
        )
    end
