...
function inputs._input:on_event(event, callback)
    ...
    if inputs.events[event] then
        event = inputs.events[event]
    end
    if event == 'error' then
...
function inputs.add(obj)
    if not is_type(obj, 'table') then
        return
    end
    if not is_type(obj.type, 'string') then
        return
    end
    local type = obj.type
    if
...
local function custom_handler(event)
    ...
    if element then
        if not is_type(element.events[event.name], 'function') then
            return
        end
        local success, err = pcall(element.events[event.name], event)
        if not success then
            if is_type(element._error, 'function') then
                pcall(element._error)
            else
                log(err)
            end
        end
...
function inputs.add_button(name, display, tooltip, callbacks)
    ...
    button:on_event(
        ...
        function(event)
            ...
            if is_type(callbacks, 'function') then
                callbacks = {
                    function(...)
                        return true
                    end,
            ...
            for _, data in pairs(callbacks) do
                if is_type(data[1], 'function') and is_type(data[2], 'function') then
                    ...
                    error('Invalid Callback Condition Format')
                end
            end
        end
    )
...
function inputs.add_checkbox(name, radio, display, default, callback_true, callback_false)
    ...
    local state = false
    if is_type(default, 'boolean') then
        state = default
    end
    local checkbox =
        inputs.add {
    ...
    if is_type(default, 'function') then
        checkbox.data._state = default
    end
